        ...
        <AppLayout title={`Detail ${managedUser.name}`}>
            <div className="glass-card mx-auto max-w-3xl p-6">
                <div className="flex items-center justify-between">
                    <div><h2 className="text-2xl font-black">{managedUser.name}</h2><p className="text-slate-500">@{managedUser.username} - {managedUser.email}</p>{managedUser.phone && <p className="mt-1 text-slate-500">HP: {managedUser.phone}</p>}</div>
                    <div className="flex gap-2"><Badge value={managedUser.role} /><Badge value={managedUser.status} /></div>
                </div>
